#!/usr/bin/env python3
"""
Run Medical-SAM3 inference on a single image using point prompts.

This script mirrors the simple single-image box workflow, but the prompt is a
set of positive point coordinates sampled from an external mask.
"""
...
def parse_points(points_text: str) -> List[Tuple[int, int]]:
    """
    Parse 'x1,y1;x2,y2;...' into a list of point coordinates.
    """
...
def scale_points(points: List[Tuple[int, int]], scale: float) -> List[Tuple[int, int]]:
    if scale == 1.0:
        return points
...
def draw_points(image: np.ndarray, points: List[Tuple[int, int]], color=(255, 99, 71), radius: int = 10) -> np.ndarray:
    plotted = Image.fromarray(image.copy())
    draw = ImageDraw.Draw(plotted)
    for x, y in points:
        draw.ellipse([x - radius, y - radius, x + radius, y + radius], fill=color, outline=color)
    return np.array(plotted)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run Medical-SAM3 on a single image with point prompts.")
    parser.add_argument("--image-path", type=Path, required=True, help="Path to the input image.")
    parser.add_argument("--points", type=str, required=True, help="Semicolon-separated XY points, e.g. '10,20;30,40'")
    ...
    original_points = parse_points(args.points)
    image, image_metadata = load_rgb_image(args.image_path, args.max_side)
    inference_points = scale_points(original_points, image_metadata["resize_scale"])
    point_labels = [1] * len(inference_points)

    output_dir = args.output_dir
    output_dir.mkdir(parents=True, exist_ok=True)

    input_with_points = draw_points(image, inference_points)

    sam3 = SAM3Model(confidence_threshold=0.1, checkpoint_path=args.checkpoint)
    inference_state = sam3.encode_image(image)
    pred_mask = sam3.predict_points(inference_state, inference_points, point_labels, image.shape[:2])
    ...
    input_path = output_dir / "input_with_points.png"
    mask_path = output_dir / "pred_mask_points.png"
    overlay_path = output_dir / "pred_overlay_points.png"
    Image.fromarray(input_with_points).save(input_path)
    ...
    summary = {
        ...
        "original_points_xy": [list(map(int, point)) for point in original_points],
        "inference_points_xy": [list(map(int, point)) for point in inference_points],
        "n_points": int(len(original_points)),
        "positive_pixels": int(pred_mask.sum()),
        "coverage_ratio": float(pred_mask.sum() / float(pred_mask.size)),
        "input_with_points": str(input_path),
        "pred_mask_points": str(mask_path),
        "pred_overlay_points": str(overlay_path),
    }
    (output_dir / "summary.json").write_text(json.dumps(summary, indent=2))

    print("=" * 60)
    print("Single-image point inference complete")
    print(f"Input with points: {input_path}")
