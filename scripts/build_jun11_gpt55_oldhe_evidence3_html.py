#!/usr/bin/env python3
"""Build shareable HTML for Jun11 GPT-5.5 old-H&E evidence-first audit."""
...
POOL_ROOT = ROOT / "output/visium_hd_exp1/final_deliverables/Jun11_GPT55_OldHE_EvidenceFirst_3/corrected_pool"
RESULT_DIR = ROOT / "output/visium_hd_exp1/final_deliverables/Jun11_GPT55_OldHE_EvidenceFirst_3/openrouter_gpt55_evidence"
OUT_DIR = ROOT / "output/visium_hd_exp1/final_deliverables/Jun11_GPT55_OldHE_EvidenceFirst_3"
OUT_HTML = OUT_DIR / "Jun11_GPT55_OldHE_EvidenceFirst_3_SHAREABLE.html"
ROOT_COPY = ROOT / "Jun11_GPT55_OldHE_EvidenceFirst_3_SHAREABLE.html"
...
def summary_table(rows: list[dict[str, str]]) -> str:
    ...
    for row in rows:
        body.append(
            "<tr>"
            f"<td><code>{esc(row['candidate_uid'])}</code></td>"
    ...
    return (
        "<table><thead><tr><th>candidate</th><th>true</th><th>GPT-5.5 pred</th><th>scores<br>bron/alv/ves/tum/stro/imm</th><th>top score reason</th></tr></thead>"
        f"<tbody>{''.join(body)}</tbody></table>"
    )
...
<title>Jun11 GPT-5.5 Old-H&E Evidence-First Audit</title><style>{css}</style></head>
<body>
<header>
  <h1>Jun11 GPT-5.5 Old-H&amp;E Evidence-First Audit</h1>
  <div class="sub">3 bronchiola candidates | old H&amp;E gray reverse-blur + FICTURE gray reverse-blur | RGB + cell type only | self-contained HTML</div>
</header>
<main>
<section>
  <h2>What This Tests</h2>
  <p><strong>Question:</strong> can GPT-5.5 classify the same three bronchiola candidates correctly when using the original H&amp;E gray reverse-blur input, while explicitly separating H&amp;E staining from the FICTURE RGB legend?</p>
  <p><strong>Prompt constraint:</strong> the FICTURE RGB legend applies only to Image 2. The prompt explicitly says H&amp;E pink/purple/blue/gray staining is not Color 0, Color 1, or any FICTURE legend color.</p>
  <p class="note"><strong>Observed result:</strong> GPT-5.5 predicted bronchiola for {correct}/{len(rows)} candidates. Unlike local Qwen, it did not collapse these examples to tumor.</p>
</section>
<section>
